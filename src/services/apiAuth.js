...
export async function getCurrentUser() {
  ...
  /*   while we could get the user here from the session,
  it is a bit more secure to just re-download everything from Supabase.  */
  const { data, error } = await supabase.auth.getUser();

  if (error) throw new Error(error.message);
  // console.log("User object from Supabase: ", data?.user);
